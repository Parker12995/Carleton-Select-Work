...
code <- nimbleCode({
  ...
  # TODO: try adding likelihood function 
...
code <- nimbleCode({
  ...
  knots_cov[1:n_knots, 1:n_knots] <- expcov(knots_D[1:n_knots, 1:n_knots], phi=0.1, sigma=sigma)
  observed_pts_vs_knots_cov[1:n_obs, 1:n_knots] <- expcov(observed_pts_vs_knots_D[1:n_obs, 1:n_knots], phi=0.1, sigma=sigma)
  ...
  mean_on_observed_pts[1:n_obs] <- observed_pts_vs_knots_cov[1:n_obs, 1:n_knots] %*% inverse(knots_cov[1:n_knots, 1:n_knots]) %*% gp_on_knots[1:n_knots]
  observed_pts_cov[1:n_obs, 1:n_obs] <- expcov(observed_pts_D[1:n_obs, 1:n_obs], phi=0.1, sigma=sigma)
  conditional_cov[1:n_obs, 1:n_obs] <- observed_pts_cov[1:n_obs, 1:n_obs] - observed_pts_vs_knots_cov[1:n_obs, 1:n_knots] %*% inverse(knots_cov[1:n_knots, 1:n_knots]) %*% t(observed_pts_vs_knots_cov[1:n_obs, 1:n_knots])
...
```{r}
plot(observed_pts, y, type="l")
points(knots, colMeans(samples)[1:ncol(samples)-1])
```
